...
// Repetidores WiFi
const MODELOS_REPETIDOR = ['ZTE ZXHN H3601P','HUAWEI K562e-10'];
...
const GESTIONES_ESPECIALES = {
  ...
  cambio_equipo: {
    ...
    campos:[
      ...
      { key:'estado',         label:'ESTADO', type:'select', options:['', 'ATENDIDA', 'PENDIENTE SE DERIVA A SISTEMAS', 'DENEGADA'] },
    ],
  },
  sot_mantto: {
  ...
  cambio_plano: {
    ...
    campos:[
      { key:'motivo_fijo',    label:'MOTIVO',             type:'fixed', value:'CAMBIO DE PLANO' },
...
function crearSeccion() {
  return {
    ...
    realizadoPor: 'ANGEL LLAMACPONCCA',
...
function guardarFormActual() {
  ...
  s.numero       = parseInt(gv('seccionNumero')) || s.numero;
  ...
  if (s.tipoGestion === 'cintillos_ftth') {
    ...
    s.modeloRepetidor   = gv('modeloRepetidor');
...
function cargarSeccion(idx) {
  ...
  sv('realizadoPor',s.realizadoPor || 'ANGEL LLAMACPONCCA');
  ...
  if (esEspecial(s.tipoGestion)) {
    ...
    sv('modeloRepetidor', s.modeloRepetidor);

    // Cargar estado select & manual text input
    const knownEstados = ['ATENDIDA', 'PENDIENTE SE DERIVA A SISTEMAS', 'DENEGADA'];
    ...
    if (s.tecnologia) {
      ...
      // Restore modelo value in select
...
function generarTextoDerivacionPext(s) {
  ...
  if (!scfg) return '';
  const L = [cfg.header, sub, `N. TICKET: ${s.nTicket||'—'}`];
  scfg.tmpl.forEach(line => L.push(line.replace(/\{(\w+)\}/g, (_, k) => vals[k]||'—')));
  L.push(`REALIZADO POR: ${s.realizadoPor||'ANGEL LLAMACPONCCA'}`);
  return L.join('\n');
}
...
function generarTextoRechazo(s) {
  ...
  if (!scfg) return '';
  const L = [cfg.header, `N. TICKET: ${s.nTicket||'—'}`];
  scfg.tmpl.forEach(line => L.push(line.replace(/\{(\w+)\}/g, (_, k) => vals[k]||'—')));
  L.push(`REALIZADO POR: ${s.realizadoPor||'ANGEL LLAMACPONCCA'}`);
  return L.join('\n');
}
...
function generarTextoEspecial(s) {
  ...
  L.push(cfg.header); L.push(`N. TICKET: ${s.nTicket||'—'}`);
    ...
    item.innerHTML = `
      <div class="section-item-inner" onclick="cambiarSeccion(${idx})">
        <div class="sec-card-top">
          <span class="sec-sot-title">SOT: ${sec.numero}</span>
...
function confirmarEliminar(idx, event) {
  ...
  if (secciones.length===1) { limpiarSeccionActual(); return; }
  secciones.splice(idx,1);
  seccionActiva = Math.max(0,Math.min(seccionActiva,secciones.length-1));
  renderSidebar(); cargarSeccion(seccionActiva);
}
...
function limpiarSeccionActual() {
  ...
  sv('realizadoPor','ANGEL LLAMACPONCCA'); sv('notasTexto','');
...
function actualizarNumero(val) {
  if (secciones.length>0) secciones[seccionActiva].numero=parseInt(val)||1;
  renderSidebar();
}
...
function convertirMAC() {
  const clean = g('macInput').value.replace(/[^A-Fa-f0-9]/g,'').slice(0,12);
  const len   = clean.length;
  const counter = g('macCounter'); const valEl = g('macValue'); const btn = g('btnCopyMAC');
  counter.textContent = `${len} / 12`;
  counter.className   = 'mac-counter'+(len===12?' ready':len>12?' over':'');
  if (len<12) { valEl.textContent=len===0?'—':`Faltan ${12-len} caracteres…`; valEl.className='mac-value'+(len>0?' error':''); btn.disabled=true; return; }
  const mac = clean.toUpperCase().match(/.{2}/g).join(':');
  valEl.textContent=mac; valEl.className='mac-value valid'; btn.disabled=false;
}
