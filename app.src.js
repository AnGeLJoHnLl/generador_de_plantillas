...
// Listener global para capturar entrada o pegado en cualquier input de serie
document.addEventListener('input', function (e) {
  const el = e.target;
  if (!el || el.tagName !== 'INPUT' || el.type !== 'text') return;

  // Excluir campos de navegación o no-seriales
  if (el.id === 'search' || el.classList.contains('cs-input') || el.id === 'seccionNumero' || el.id === 'realizadoPor' || el.id === 'codeEUser') return;
